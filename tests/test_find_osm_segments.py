...
class FindOsmSegmentsTests(unittest.TestCase):
    ...
    def test_calculate_distance_handles_connect_failure_without_cleanup_when_temp_missing(self):
        generator = random.Random(703)
        temp_suffix = generator.randint(10_000, 99_999)

        with (
            mock.patch.object(self.module.duckdb, "connect", side_effect=RuntimeError("connect fail"), create=True),
            mock.patch.object(self.module.random, "randint", return_value=temp_suffix),
...
class FindOsmSegmentsMainTests(unittest.TestCase):
    ...
    def test_main_returns_when_metadata_getmtime_raises_oserror(self):
        filename = "metadata_unfiltered_159-145-8.parquet"
    ...
    def test_main_raises_when_updated_after_is_invalid(self):
        cfg = self._cfg()
    ...
    def test_main_raises_when_metadata_params_section_missing(self):
        cfg = self._cfg()
        cfg.pop("metadata_params", None)
        filename = "metadata_unfiltered_159-145-8.parquet"
        calls = []
    ...
    def test_main_exits_when_tile_cannot_be_extracted(self):
        with (
            mock.patch.object(self.module, "load_config", return_value=self._cfg(), create=True),
            mock.patch.object(self.module.sys, "argv", ["find_osm_segments.py", "metadata_unfiltered_invalid.parquet"]),
            mock.patch.object(self.module.os, "chdir"),
        ):
            with self.assertRaises(SystemExit):
                self.module.main()

    def test_main_skips_distance_loop_when_no_osm_parquet_files(self):
        filename = "metadata_unfiltered_159-145-8.parquet"
        ...
        with (
            ...
            mock.patch.object(self.module.os.path, "isdir", return_value=True),
            mock.patch.object(self.module.os.path, "getmtime", return_value=32503680000),
            mock.patch.object(self.module.os, "listdir", return_value=["x.txt"]),
            mock.patch.object(self.module, "calculate_distance") as calc_mock,
        ):
            self.module.main()
    ...
    def test_main_returns_early_when_metadata_is_older_than_updated_after(self):
        cfg = self._cfg()
        cfg["metadata_params"]["updated_after"] = "2100-01-01T00:00:00"
        filename = "metadata_unfiltered_159-145-8.parquet"
    ...
    def test_module_entrypoint_invokes_main_when_run_as_script(self):
        fake_numpy = types.ModuleType("numpy")
        fake_numpy.radians = lambda values: values
        fake_numpy.sin = lambda value: value
        fake_numpy.cos = lambda value: value
        fake_numpy.arctan2 = lambda left, right: 0
        ...
        with (
            mock.patch.dict(sys.modules, {"numpy": fake_numpy, "duckdb": fake_duckdb, "start": fake_start}),
            mock.patch("os.chdir"),
            mock.patch.object(sys, "argv", ["find_osm_segments.py"]),
        ):
            with self.assertRaises(SystemExit):
                runpy.run_module("find_osm_segments", run_name="__main__")
